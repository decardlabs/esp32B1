...
extern void ble_hid_task_start_up(void);
extern void hid_on_security_changed(bool secure);
extern void hid_on_disconnected(void);
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    /* Initialize security for phone compatibility: Just Works + bonding. */
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        hid_on_security_changed(false);
        /* Reset connection state and restart advertising directly in GAP handler.
         * Do NOT rely solely on ESP_HIDD_DISCONNECT_EVENT (HID layer) for this —
         * the HID event may not fire reliably with CONFIG_BT_NIMBLE_HID_SERVICE
         * enabled alongside esp_hidd_dev_init(). Restarting advertising here also
         * prevents auto-reconnect races on the peer side. */
        hid_on_disconnected();
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        ...
        assert(rc == 0);
        hid_on_security_changed(event->enc_change.status == 0);
        ble_hid_task_start_up();
        return 0;

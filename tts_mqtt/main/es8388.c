...
/* ES8388 register map (DAC/playback subset) */
...
esp_err_t es8388_set_volume(int vol_db)
{
    ...
    uint8_t dac_reg = (uint8_t)((-vol_db) * 2);
...
esp_err_t es8388_init(void)
{
    ...
    /* Power up and configure DAC path for playback */
    ...
    /* Disable ADC path (not needed for playback-only) */
    ret |= reg_write(ES8388_ADCPOWER, 0xFF);
    ret |= reg_write(ES8388_DACPOWER, 0x3C);       /* DAC power on */

    ret |= es8388_set_volume(-12);
    ret |= es8388_mute(true);                       /* start muted */

    ESP_RETURN_ON_ERROR(ret, TAG, "codec init failed");
    ESP_LOGI(TAG, "ES8388 initialized, volume=-12dB, muted");
    return ESP_OK;
}

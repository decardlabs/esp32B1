...
static esp_err_t reg_write_unlocked(uint8_t reg, uint8_t data)
{
...
static esp_err_t reg_read_unlocked(uint8_t reg, uint8_t *data)
{
    esp_err_t ret = i2c_master_transmit_receive(s_dev, &reg, 1, data, 1, pdMS_TO_TICKS(100));
    return ret;
}
...
static esp_err_t pin_set_dir_output(uint8_t port, uint8_t pin)
{
    ...
    uint8_t val;
    i2c_bus_lock();
    esp_err_t ret = reg_read_unlocked(reg, &val);
    if (ret != ESP_OK) {
        i2c_bus_unlock();
        return ret;
    }
    val &= ~(1 << pin); /* 0 = output */
    ret = reg_write_unlocked(reg, val);
    i2c_bus_unlock();
    return ret;
}

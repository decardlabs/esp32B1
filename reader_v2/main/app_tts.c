...
/* Static FreeRTOS buffers (§1.1) */
#define TTS_QUEUE_LEN 4
static StaticQueue_t s_work_queue_buf;
static uint8_t s_work_queue_storage[TTS_QUEUE_LEN * sizeof(tts_work_t)];
static StackType_t s_worker_stack[TTS_WORKER_STACK];
...
static void tts_worker_task(void *arg)
{
    ...
    while (1) {
        ...
        if (loop_cnt % 8 == 0) {
            ...
            ESP_LOGI(TAG, "[STACK] task=tts_work watermark=%u words", (unsigned)watermark);
        }
...
esp_err_t app_tts_init(void)
{
    ...
    /* Create work queue (static) and persistent worker task (§1.1) */
    s_work_queue = xQueueCreateStatic(TTS_QUEUE_LEN, sizeof(tts_work_t),
                                       s_work_queue_storage, &s_work_queue_buf);
    if (!s_work_queue) {
        ESP_LOGE(TAG, "work queue create failed");
        return ESP_ERR_NO_MEM;
    }
    s_worker_task = xTaskCreateStatic(tts_worker_task, "tts_work", TTS_WORKER_STACK,
                                       NULL, TTS_WORKER_PRIO,
...
/* ── I2S handle sharing (for cloud TTS) ─────────────────────────── */

void *app_tts_get_i2s_handle(void)
{
    if (s_tx_handle == NULL) {
        ESP_LOGE(TAG, "I2S handle requested but not initialized");
    }
    return (void *)s_tx_handle;
}

...
/* GPIO2 may be shared with other board functions; keep it high to avoid interference */
...
static esp_lcd_panel_io_callbacks_t s_lcd_io_callbacks DRAM_ATTR = {0};
static void *s_lcd_io_user_ctx DRAM_ATTR = NULL;
static volatile lcd_st7796_transfer_t s_pending_transfer DRAM_ATTR = LCD_ST7796_TRANSFER_NONE;
static volatile lcd_st7796_transfer_t s_completed_transfer DRAM_ATTR = LCD_ST7796_TRANSFER_NONE;

static bool IRAM_ATTR lcd_st7796_color_done_cb(esp_lcd_panel_io_handle_t panel_io,
...
lcd_st7796_transfer_t IRAM_ATTR lcd_st7796_take_completed_transfer_from_isr(void)
{
...
static esp_err_t lcd_st7796_bus_init(void)
{
    ...
    esp_lcd_panel_io_spi_config_t io_cfg = {
        .dc_gpio_num = BSP_GPIO_LCD_DC,
        .cs_gpio_num = BSP_GPIO_LCD_CS,
...
esp_err_t lcd_st7796_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(xl9555_lcd_control_init(), TAG, "lcd gpio init failed");
    ...
    ESP_RETURN_ON_ERROR(xl9555_backlight_on(), TAG, "lcd backlight on failed");
...
{
    return xl9555_backlight_on();
}

esp_err_t lcd_st7796_backlight_off(void)
{
    return xl9555_backlight_off();
}

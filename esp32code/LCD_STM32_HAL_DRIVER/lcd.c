...
//   }
	 ...
	 LCD_Writ_Bus_Len((uint8_t *)color,width*height*2);
}
...
//		}
		...
		HAL_SPI_Transmit(&hspi1, (uint8_t *)color_p, 2, 100);
...
//	SPI_Cmd(SPI1, ENABLE);//使能SPI
//	SPI1_ReadWriteByte(0xff);//DMA执行完之后需要发送一个空字节，否则触摸不响应
}
...
//			}
//		}
		HAL_SPI_Transmit(&hspi1, pic, num1, 100);
		pic+=65534;
	}
	LCD_CS_Set();
	//SPI1_ReadWriteByte(0xff);//DMA执行完之后需要发送一个空字节，否则触摸不响应

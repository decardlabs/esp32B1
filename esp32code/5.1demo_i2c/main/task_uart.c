...
static const char *TAG = "TASK_UART";  // 日志标签
...
extern QueueHandle_t ledc_queue_handle;
/**
 * @brief 计算数据校验和
 * 
...
void process_frame(const uint8_t *data, uint16_t len)
{
    ...
    uint32_t val = data[1];   // 提取参数值（第二个字节）
    ...
    switch (cmd)
    {
        case 0x01:  // 控制 LED 命令
            // 根据参数值控制LED状态
            val = val * 4;    
            xQueueSend(ledc_queue_handle, &val, portMAX_DELAY);
            // 退出switch语句，结束本次命令处理
            break;

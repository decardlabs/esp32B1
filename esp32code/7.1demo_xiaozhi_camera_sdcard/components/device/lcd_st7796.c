...
#define LCD_PIN_NUM_TOUCH_CS    GPIO_NUM_2
#define LCD_PIN_NUM_TOUCH_PEN   GPIO_NUM_8
...
static void lcd_st7796_power_gpio_init(void)
{
    xl9555_set_pin_level(TFT_PORT, TFT_BLK_PIN, 0);
}
...
static void lcd_st7796_ctrl_gpio_init(void)
{
    ...
    gpio_config_t pen_cfg = {
        .pin_bit_mask = 1ULL << LCD_PIN_NUM_TOUCH_PEN,
...
static esp_err_t lcd_st7796_bus_init(void)
{
    ...
    if (!bsp_spi2_lcd_is_initialized()) {
    ...
    return esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)bsp_get_spi2_lcd_host(), &io_cfg, &lcd_io_handle);
}

static void lcd_st7796_reset(void)
{
    xl9555_set_pin_level(TFT_PORT, TFT_RES_PIN, 0);
    vTaskDelay(pdMS_TO_TICKS(100));
    xl9555_set_pin_level(TFT_PORT, TFT_RES_PIN, 1);
    vTaskDelay(pdMS_TO_TICKS(100));
}
...
esp_err_t lcd_st7796_register_event_callbacks(const esp_lcd_panel_io_callbacks_t *callbacks, void *user_ctx)
{
    ...
    return esp_lcd_panel_io_register_event_callbacks(lcd_io_handle,
                                       ...
                                       lcd_st7796_transfer_t transfer_type)
{
    ...
    if (esp_lcd_panel_io_tx_param(lcd_io_handle, LCD_CMD_CASET, x_param, sizeof(x_param)) != ESP_OK) {
        s_pending_transfer = LCD_ST7796_TRANSFER_NONE;
        bsp_spi2_bus_unlock();
        return ESP_FAIL;
    }
    if (esp_lcd_panel_io_tx_param(lcd_io_handle, LCD_CMD_RASET, y_param, sizeof(y_param)) != ESP_OK) {
        s_pending_transfer = LCD_ST7796_TRANSFER_NONE;
        bsp_spi2_bus_unlock();
        return ESP_FAIL;
    }

    esp_err_t ret = esp_lcd_panel_io_tx_color(lcd_io_handle, LCD_CMD_RAMWR, color_data, color_size);
    if (ret != ESP_OK) {
        s_pending_transfer = LCD_ST7796_TRANSFER_NONE;
        bsp_spi2_bus_unlock();
    }
...
{
    xl9555_set_pin_level(TFT_PORT, TFT_BLK_PIN, 1);
    return ESP_OK;
}
...
bool lcd_st7796_touch_pressed(void)
{
    return gpio_get_level(LCD_PIN_NUM_TOUCH_PEN) == 0;
}
